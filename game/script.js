...
,
  {
    title: "Fase 6 — Cristal Dourado",
    subtitle: "Mais pontos e mais planejamento para revelar a memória.",
    themeText: "Fundo dourado com tabuleiro recortado para aumentar o desafio.",
    pointsGoal: 2600,
    moves: 34,
    cactusCount: 7,
    revealCover: 92,
    revealFactor: 0.9,
    cactusRevealFactor: 2,
    revealStartProgress: 0.75,
    finalRevealBoost: 4,
    theme: { bgStart: "#fff36b", bgMid: "#ff9d00", bgEnd: "#8b2d00", boardBg1: "rgba(22,22,22,.98)", boardBg2: "rgba(0,0,0,.96)", frame1: "rgba(255,255,255,.40)", frame2: "rgba(110,110,110,.30)" },
    layout: ["11111111","11111111","11011011","11111111","11111111","11011011","11111111","11111111"]
  },
  {
    title: "Fase 7 — Safira Rosa",
    subtitle: "Fase premium com passagens estreitas.",
    themeText: "Visual rosa e azul com corredores que exigem combinações certeiras.",
    pointsGoal: 3000,
    moves: 36,
    cactusCount: 8,
    revealCover: 94,
    revealFactor: 0.85,
    cactusRevealFactor: 2,
    revealStartProgress: 0.76,
    finalRevealBoost: 4.2,
    theme: { bgStart: "#ff7ac8", bgMid: "#8f2dff", bgEnd: "#21006b", boardBg1: "rgba(22,22,22,.98)", boardBg2: "rgba(0,0,0,.96)", frame1: "rgba(255,255,255,.40)", frame2: "rgba(110,110,110,.30)" },
    layout: ["11100111","11111111","01111110","11111111","11111111","01111110","11111111","11100111"]
  },
  {
    title: "Fase 8 — Baile da Valentina",
    subtitle: "Última fase para fechar a coleção premium.",
    themeText: "Fase final com brilho de baile, mais cactos e revelação no fim.",
    pointsGoal: 3500,
    moves: 38,
    cactusCount: 9,
    revealCover: 96,
    revealFactor: 0.8,
    cactusRevealFactor: 2,
    revealStartProgress: 0.78,
    finalRevealBoost: 4.8,
    theme: { bgStart: "#ffe36c", bgMid: "#ff1493", bgEnd: "#5b0038", boardBg1: "rgba(22,22,22,.98)", boardBg2: "rgba(0,0,0,.96)", frame1: "rgba(255,255,255,.48)", frame2: "rgba(255,215,80,.34)" },
    layout: ["00111100","01111110","11111111","11111111","11111111","11111111","01111110","00111100"]
  }
];

const CONFIG = window.FIGURINHAS_CONFIG || { total: 36, arquivos: [] };
...
function buildStickerFileList() {
  ...
  return Array.from({ length: total }, (_, i) => `${String(i + 1).padStart(3, "0")}.webp`);
}
...
function toggleAudio() {
  setGameAudioState(!audioOn);
}

window.setGameAudioState = setGameAudioState;
...
function loadStickerImage() {
  ...
  const stickerNumText = String(currentStickerNumber).padStart(3, "0");
  ...
  stickerImageEl.src = `figurinhas/${fileName}`;
...
function notifyAlbumPhaseWon() {
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'VALENTINA_GAME_PHASE_WON', phase: phaseIndex + 1 }, '*');
    }
  } catch (e) {}
}
...
function checkWin() {
  ...
  if (score >= phase.pointsGoal && cactusBroken >= cactusTarget && getCoveredCount() === 0) {
    ...
    if (phaseIndex >= PHASES.length - 1) {
      openModal(
        "Missão concluída!",
        "Você concluiu todas as fases. Volte ao álbum para ver seus pacotes liberados sem repetidas.",
        { primaryText: "Reiniciar jogo", primaryAction: () => { closeModal(); resetGame(); } }
      );
    } else {
      openModal(
        "Fase vencida!",
        "Você concluiu a fase e pode avançar para o próximo layout.",
...
function checkEndByMoves() {
  ...
  openModal(
    "Fim das jogadas",
    "As jogadas acabaram. A dificuldade foi mantida, mas agora você pode reiniciar esta fase sem travar o jogo.",
    {
...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'VALENTINA_GAME_AUDIO') {
    setGameAudioState(!!event.data.on);
  }
});
